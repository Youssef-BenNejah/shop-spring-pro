    ...
    >
        ...
        <div className="relative aspect-[3/4] bg-secondary rounded-lg overflow-hidden mb-3">
          ...
          {!product.inStock && (
            <div className="absolute inset-0 bg-background/60 flex items-center justify-center">
              <span className="font-body text-sm font-medium text-foreground">Sold Out</span>
            </div>
          )}
      ...
      <div className="space-y-1">
        ...
        <Link to={`/product/${product.slug}`}>
          <h3 className="font-body text-sm font-medium text-foreground leading-snug group-hover:text-accent transition-colors">
            {product.name}
          </h3>
        </Link>
        <div className="flex items-center gap-2">
          ...
          {product.compareAtPrice && (
            <span className="font-body text-xs text-muted-foreground line-through">${product.compareAtPrice}</span>
          )}
        </div>
